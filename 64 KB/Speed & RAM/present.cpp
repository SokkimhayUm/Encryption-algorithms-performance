...
// Benchmark
int main() {
    const size_t dataSize = 64 * 1024; // 64 KB
    ...
    double totalEncTime = 0.0, totalDecTime = 0.0;
    bool passed = true;
    ...
    double totalDataMB = (dataSize * iterations) / (1024.0 * 1024.0);
    double encMBps = totalDataMB / totalEncTime;
    double decMBps = totalDataMB / totalDecTime;

    std::cout << std::fixed << std::setprecision(6);
    std::cout << "=== Present Benchmark ===\n";
    std::cout << "PRESENT ran " << iterations << " times on " << (dataSize / 1024.0) << " KB blocks\n";
    std::cout << "Total data processed: " << totalDataMB << " MB\n";
    std::cout << "Present Encryption time (100x): " << totalEncTime << " sec, " << encMBps << " MB/s\n";
    std::cout << "Present Decryption time (100x): " << totalDecTime << " sec, " << decMBps << " MB/s\n";
